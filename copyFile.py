...
w = os.get_terminal_size().columns // 3
...
def copy_progress(copied, total):
    print('\r' + progress_percentage(100*copied/total, width=w), end='')
    ...
    if not follow_symlinks and os.path.islink(src):
        ...
        with open(src, 'rb') as fsrc:
            with open(dst, 'wb') as fdst:
                hash = copyfileobj(fsrc, fdst, callback=copy_progress, total=size)
    return hash


def copyfileobj(fsrc, fdst, callback, total, length=blocks):
    copied = 0
    md5s = hashlib.md5()
    md5d = hashlib.md5()
    while True:
        ...
        if not buf:
            break
        fdst.write(buf)
        # cannot read destination hash here because 'wb'
        md5s.update(buf)
        copied += len(buf)
...
def copy_with_progress(src, dst, follow_symlinks):
    if os.path.isdir(dst):
        #print(dst)
        dst = os.path.join(dst, os.path.basename(src))
    h = copyfile(src, dst, follow_symlinks=follow_symlinks)
